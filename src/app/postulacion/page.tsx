...
function PostulacionInner() {
  ...
  useEffect(() => {
    const fetchVacante = async () => {
      ...
      try {
        const response = await fetch(`/api/vacantes/${vacanteId}`);
        if (response.ok) {
    ...
    return (
      <section className="w-full min-h-screen bg-white mb-16">
        <div className="flex justify-center items-center py-20">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#0D4A7A]"></div>
        </div>
      </section>
    );
  }
      ...
      <section className="w-full min-h-screen bg-white mb-16">
        <div className="flex justify-center items-center py-20">
          <div className="text-center">
            <h1 className="text-2xl font-bold text-gray-900 mb-4">Vacante no encontrada</h1>
            <p className="text-gray-600">La vacante solicitada no existe o ha sido removida.</p>
          </div>
        </div>
      </section>
    );
...
function MultiStepForm({ vacante }: { vacante: Vacante }) {
