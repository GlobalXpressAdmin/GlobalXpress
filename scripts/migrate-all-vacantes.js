...
// Datos de vacantes desde vacantesData.ts
const vacantesData = [
  {
...
async function migrateVacantes() {
  try {
    ...
    // Verificar si ya existen vacantes en la base de datos
    const existingVacantes = await prisma.vacante.count();
    console.log(`Vacantes existentes en la base de datos: ${existingVacantes}`);
    
    if (existingVacantes > 0) {
      console.log('Ya existen vacantes en la base de datos. ¿Deseas continuar y agregar las nuevas? (Ctrl+C para cancelar)');
    ...
    for (const vacante of vacantesData) {
      try {
        // Verificar si la vacante ya existe (por empresa y cargo)
        ...
        if (existingVacante) {
          console.log(`Vacante ya existe: ${vacante.empresa} - ${vacante.cargo}`);
          skippedCount++;
          continue;
        }
        ...
        await prisma.vacante.create({
          data: {
            ...
            descripcion: vacante.descripcion || null,
            email: vacante.email === 'No especificado' ? null : vacante.email,
            workers: vacante.workers === 'No especificado' ? null : vacante.workers,
            link: vacante.link === 'No especificado' ? null : vacante.link,
            activa: true, // Todas las vacantes se crean como activas
          }
        });
        
        console.log(`✓ Creada: ${vacante.empresa} - ${vacante.cargo}`);
        createdCount++;
        
      } catch (error) {
        console.error(`Error al crear vacante ${vacante.empresa}:`, error);
      }
    }
    
    console.log('\n=== Resumen de migración ===');
    console.log(`Vacantes creadas: ${createdCount}`);
    console.log(`Vacantes omitidas (ya existían): ${skippedCount}`);
    console.log(`Total procesadas: ${createdCount + skippedCount}`);
    
    // Verificar el total final
    const totalVacantes = await prisma.vacante.count();
    console.log(`Total de vacantes en la base de datos: ${totalVacantes}`);
